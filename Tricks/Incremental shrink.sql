...
DECLARE file_cursor CURSOR FAST_FORWARD LOCAL FORWARD_ONLY FOR 
    SELECT name FROM sys.master_files WHERE type_desc = 'ROWS' AND database_id = DB_ID(@DbName);
...
BEGIN
    SELECT @FileSizeMB = size/128., @UsedMB = FILEPROPERTY(@LogicalFileName, 'SpaceUsed')/128. FROM sys.master_files WHERE name = @LogicalFileName AND database_id = DB_ID(@DbName);
